...
export default function exchangeRatesStoreDaemonFactory(bity) {
  return function* runExchangeRatesStoreDaemon() {
    yield [
...
function* sendFetchDataRequest(bity) {
  try {
    const data = yield call(bity.exchangeRates.fetchAllRates);
